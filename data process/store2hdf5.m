function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/lr', [dat_dims(1:end-1) Inf], 'Datatype', 'double', 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label', [lab_dims(1:end-1) Inf], 'Datatype', 'double', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
    h5write(filename, '/lr', single(data), startloc.dat, size(data));
